...
//For computing percent error. This should not be used for determining equality of floating
// point values. See RELATIVE_DIFF for this purpose.
#ifndef PERCENT_ERR
...
*/
template<typename ToType, typename FromType> ToType Ygor_Pack_As(FromType in){
...
//Generate a random [0-9a-zA-Z] string of length len. Useful for finding an unused filename.
//void gen_random(std::string *s, const int len);   //NOTE: An alternative, better implementation is in YgorString.h/cc. Use it if possible!

//Seeds the (old-style) random number generator with the current time.
//void seed_time(void);

//Prints out a set. This should be turned into an iostream form, but it was only needed briefly.
