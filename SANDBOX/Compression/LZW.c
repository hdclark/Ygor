...
{
        if(argc != 2){
            fprintf(stderr, "You need to specify the test file to compress.\n");
            return 1;
        }
	...
	close(fd);
        ...
        int orig_size = _len(in); 
	printf("Input size = %d.\n", _len(in));

int varbitlength;
for(varbitlength = 9; varbitlength < 16; ++varbitlength){
        printf("  Var bit len = %d.    ", varbitlength); 

	byte *enc = lzw_encode(in, varbitlength);
	printf("Enc size: %d or %f%     ", _len(enc), _len(enc)*100.0/orig_size);
 
	byte *dec = lzw_decode(enc);
//	printf("Dec size: %d.    ", _len(dec));

        if(orig_size != _len(dec)){
            fprintf(stderr, "--(E) Error encountered!");
            return 1;
        } 
