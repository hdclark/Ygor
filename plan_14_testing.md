...
Root `CMakeLists.txt` currently adds GCC `-ffast-math`. Components 1-14 already require a named strict Boolean source/test policy. Ensure effective Boolean compilation removes fast-math/finite-math assumptions and contraction where required, adds compile-time rejection of `__FAST_MATH__`, and preserves signed zero, subnormals, and floating-environment tests. Inspect `compile_commands.json` in qualification; later flags must not silently re-enable unsafe behavior.
...
Apply strict C++17 and strict floating flags target-locally to every Boolean production, oracle, replay, and test target. Verify `__cplusplus`, reject `__FAST_MATH__`, and add supported compiler flags that disable unsafe contraction/finite assumptions. Do not globally remove flags needed by unrelated Ygor targets unless separately approved.
