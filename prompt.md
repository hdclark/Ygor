...
You are in the final phase of a comprehensive plan to implementing a robust b-rep surface mesh boolean engine. Design and specification is complete, and now the plan needs to be carefully implemented. The overall plan is in `broad_plan.md`, which outlines goals and several individual components. A detailed, precise, and specific plan for each individual component has been written to files `plan_N_name.md` where `N` is the component number and `name` is a shorthand name.
