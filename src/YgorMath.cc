...
int insphere_sign(const vec3<T> &a,
    ...
    //// Direct way, which does not handle (1) vertices offset from the origin or (2) vertices with
    //// small offsets well.
    //const std::array<T, 3> pa{{ a.x, a.y, a.z }};
    //const std::array<T, 3> pb{{ b.x, b.y, b.z }};
    //const std::array<T, 3> pc{{ c.x, c.y, c.z }};
    ...
    // Translate all points using vertex 'd' and then compare.
    const auto aex = a.x - e.x;
    const auto aey = a.y - e.y;
    const auto aez = a.z - e.z;
    const auto bex = b.x - e.x;
    const auto bey = b.y - e.y;
    const auto bez = b.z - e.z;
    const auto cex = c.x - e.x;
    const auto cey = c.y - e.y;
    const auto cez = c.z - e.z;
    const auto dex = d.x - e.x;
    const auto dey = d.y - e.y;
    const auto dez = d.z - e.z;

    const std::array<T, 3> pa{{ aex, aey, aez }};
    const std::array<T, 3> pb{{ bex, bey, bez }};
    const std::array<T, 3> pc{{ cex, cey, cez }};
    const std::array<T, 3> pd{{ dex, dey, dez }};
    const std::array<T, 3> pe{{ static_cast<T>(0), static_cast<T>(0), static_cast<T>(0) }};
...
struct mesh_intersection_triangle {
    size_t face_index = 0;
    std::array<size_t, 3> vertex_indices;
...
std::vector<mesh_intersection_triangle<T>>
collect_intersection_triangles(const fv_surface_mesh<T, I> &mesh){
    std::vector<mesh_intersection_triangle<T>> out;
    ...
    for(size_t face_idx = 0; face_idx < mesh.faces.size(); ++face_idx){
        ...
        for(size_t i = 1UL; i + 1UL < face.size(); ++i){
            const std::array<size_t, 3> vertex_indices = {{
                static_cast<size_t>(face.at(0)),
                static_cast<size_t>(face.at(i)),
                static_cast<size_t>(face.at(i + 1UL))
            }};
            const std::array<vec3<T>, 3> tri = {{
                mesh.vertices.at(vertex_indices.at(0)),
                mesh.vertices.at(vertex_indices.at(1)),
                mesh.vertices.at(vertex_indices.at(2))
            }};
            if(!(triangle_normal(tri).sq_length() > static_cast<T>(0))){
                continue;
            }

            mesh_intersection_triangle<T> mt;
            mt.face_index = face_idx;
...
inline void
sort_and_dedup_primitives(std::vector<fv_surface_mesh_intersection_primitive> &primitives){
...
template <class T>
std::vector<fv_surface_mesh_intersection_primitive>
collect_triangle_primitives(const mesh_intersection_triangle<T> &tri,
                            const fv_surface_mesh_intersection_geometry<T> &geometry,
                            T eps){
    std::vector<fv_surface_mesh_intersection_primitive> out;
    out.push_back(fv_surface_mesh_intersection_primitive{
    ...
    auto add_edge_if_active = [&](size_t i, size_t j){
        ...
        if(active){
            auto edge = std::vector<size_t>{ tri.vertex_indices.at(i), tri.vertex_indices.at(j) };
...
template <class T>
bool
intersection_less(const fv_surface_mesh_intersection<T> &lhs,
                  const fv_surface_mesh_intersection<T> &rhs){
...
template <class T>
bool
intersections_equal(const fv_surface_mesh_intersection<T> &lhs,
                    const fv_surface_mesh_intersection<T> &rhs){
    const auto primitive_equal = [](const fv_surface_mesh_intersection_primitive &lp,
                                    const fv_surface_mesh_intersection_primitive &rp){
...
fv_surface_mesh<T,I>::intersections_with(const fv_surface_mesh<T, I> &other,
                                         T snap_eps) const {
    std::vector<fv_surface_mesh_intersection<T>> out;

    const auto eps = mesh_intersection_eps(*this, other, snap_eps);
    const auto lhs_tris = collect_intersection_triangles(*this);
    const auto rhs_tris = collect_intersection_triangles(other);
    ...
    std::sort(out.begin(), out.end(), intersection_less<T>);
    out.erase(std::unique(out.begin(), out.end(), intersections_equal<T>), out.end());
    return out;
}
#ifndef YGORMATH_DISABLE_ALL_SPECIALIZATIONS
    template std::vector<fv_surface_mesh_intersection<float >>
        fv_surface_mesh<float , uint32_t>::intersections_with(const fv_surface_mesh<float , uint32_t> &, float ) const;
    template std::vector<fv_surface_mesh_intersection<float >>
        fv_surface_mesh<float , uint64_t>::intersections_with(const fv_surface_mesh<float , uint64_t> &, float ) const;

    template std::vector<fv_surface_mesh_intersection<double>>
        fv_surface_mesh<double, uint32_t>::intersections_with(const fv_surface_mesh<double, uint32_t> &, double) const;
    template std::vector<fv_surface_mesh_intersection<double>>
        fv_surface_mesh<double, uint64_t>::intersections_with(const fv_surface_mesh<double, uint64_t> &, double) const;
#endif
...
//---------------------------------------------------------------------------------------------------------------------------
//----------------- fv_tet_mesh: a volumetric tetrahedral mesh with a straightforward data structure ------------------------
//---------------------------------------------------------------------------------------------------------------------------
