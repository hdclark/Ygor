...
struct csv_load_result {
    ...
    // Bidirectional mapping from each distinct non-numeric string to its assigned
    // integer value.
    //
    // These are only populated by the default non-numeric callback. User-provided
    // may or may not use these.
    std::map<std::string, int64_t> string_to_int;
    std::map<int64_t, std::string> int_to_string;
