...
template<class Draft,class Artifact>class stage_transaction{
    context_owner_token owner_;boolean_stage stage_;artifact_slot slot_;transaction_state state_=transaction_state::open;std::unique_ptr<Draft>draft_;std::shared_ptr<const Artifact>candidate_;verification_report report_;std::unique_ptr<publication_certificate>certificate_;std::vector<resource_reservation>reservations_;
public:
    stage_transaction(context_owner_token o,boolean_stage s,artifact_slot slot,std::unique_ptr<Draft>d):owner_(o),stage_(s),slot_(slot),draft_(std::move(d)){}
    ...
    status_or<bool>verify(std::shared_ptr<const Artifact>a,const artifact_view&v,const verification_spec&spec,const verification_environment_view&env,const verifier_service&service){if(state_!=transaction_state::open)return make_error(boolean_error_code::internal_invariant_error,stage_,"transaction_state");auto r=service.verify(v,spec,env);if(!r.has_value()||!r.value().passed()){state_=transaction_state::failed;return r.has_value()?make_error(boolean_error_code::internal_invariant_error,stage_,"verification_failed"):r.error();}if(r.value().owner!=owner_||r.value().stage!=stage_||r.value().slot!=slot_||r.value().artifact_digest!=v.artifact_digest||r.value().artifact_type_tag!=v.artifact_type_tag||r.value().artifact_schema!=v.artifact_schema||r.value().invariant_set_digest!=spec.invariant_set_digest){state_=transaction_state::failed;return make_error(boolean_error_code::internal_invariant_error,stage_,"verification_binding");}candidate_=std::move(a);report_=std::move(r.value());certificate_.reset(new publication_certificate(owner_,stage_,slot_,v.artifact_type_tag,v.artifact_schema,v.artifact_digest,report_.report_digest,spec.invariant_set_digest));state_=transaction_state::verified;return true;}
