...
class publication_certificate{
    ...
    template<class A>friend class stage_transaction;
...
template<class Artifact>class stage_transaction{
    context_owner_token owner_;boolean_stage stage_;artifact_slot slot_;performance_collector*performance_=nullptr;boolean_stage performance_verifier_stage_;transaction_state state_=transaction_state::open;std::unique_ptr<Artifact>draft_;std::shared_ptr<const Artifact>candidate_;verification_report report_;std::unique_ptr<publication_certificate>certificate_;std::vector<resource_reservation>reservations_;
    void fail()noexcept{state_=transaction_state::failed;certificate_.reset();candidate_.reset();reservations_.clear();}
public:
    stage_transaction(context_owner_token o,boolean_stage s,artifact_slot slot,std::unique_ptr<Artifact>d,performance_collector*p=nullptr):owner_(o),stage_(s),slot_(slot),performance_(p),performance_verifier_stage_(s),draft_(std::move(d)){if(!draft_)throw std::invalid_argument("null transaction draft");}
    stage_transaction(context_owner_token o,boolean_stage s,artifact_slot slot,std::unique_ptr<Artifact>d,performance_collector*p,boolean_stage verifier_stage):owner_(o),stage_(s),slot_(slot),performance_(p),performance_verifier_stage_(verifier_stage),draft_(std::move(d)){if(!draft_)throw std::invalid_argument("null transaction draft");}
    stage_transaction(stage_transaction&&)=default;stage_transaction(const stage_transaction&)=delete;transaction_state state()const{return state_;}Artifact&draft(){if(state_!=transaction_state::open||!draft_)throw std::logic_error("transaction not open");return*draft_;}
    ...
    status_or<bool>freeze_and_verify(std::uint64_t type,std::uint16_t schema,std::uint64_t generation,const digest&artifact_digest,const verification_spec&spec,const verification_environment_view&env,const verifier_service&service){if(state_!=transaction_state::open||!draft_)return make_error(boolean_error_code::internal_invariant_error,stage_,"transaction_state");candidate_=std::shared_ptr<const Artifact>(std::move(draft_));artifact_view v{owner_,slot_,type,schema,generation,artifact_digest,candidate_,candidate_.get()};performance_scope timing(performance_,performance_verifier_stage_,performance_role::verifier);auto r=service.verify(v,spec,env);timing.finish();if(!r.has_value()||!r.value().passed()){auto error=r.has_value()?make_error(boolean_error_code::internal_invariant_error,stage_,"verification_failed"):r.error();fail();return error;}if(r.value().owner!=owner_||r.value().stage!=stage_||r.value().slot!=slot_||r.value().artifact_digest!=v.artifact_digest||r.value().artifact_type_tag!=v.artifact_type_tag||r.value().artifact_schema!=v.artifact_schema||r.value().invariant_set_digest!=spec.invariant_set_digest){fail();return make_error(boolean_error_code::internal_invariant_error,stage_,"verification_binding");}report_=std::move(r.value());certificate_.reset(new publication_certificate(owner_,stage_,slot_,v.artifact_type_tag,v.artifact_schema,v.artifact_digest,report_.report_digest,spec.invariant_set_digest));state_=transaction_state::verified;return true;}
    status_or<std::shared_ptr<const published_artifact<Artifact>>>publish(){if(state_!=transaction_state::verified||!certificate_||certificate_->consumed_||certificate_->owner_!=owner_||certificate_->stage_!=stage_||certificate_->slot_!=slot_||certificate_->artifact_!=report_.artifact_digest||certificate_->report_!=report_.report_digest){fail();return make_error(boolean_error_code::internal_invariant_error,stage_,"publish_unverified");}certificate_->consumed_=true;auto p=std::make_shared<published_artifact<Artifact>>(published_artifact<Artifact>{owner_,stage_,slot_,report_.artifact_digest,candidate_,report_,1,{}});for(auto&r:reservations_)r.commit();state_=transaction_state::published;candidate_.reset();return std::shared_ptr<const published_artifact<Artifact>>(std::move(p));}
    status_or<std::shared_ptr<const published_artifact<Artifact>>>compare_and_publish(artifact_generation_catalog&catalog,std::uint64_t expected,std::shared_ptr<const void>prior={}){if(state_!=transaction_state::verified||!certificate_||certificate_->consumed_||certificate_->owner_!=owner_||certificate_->stage_!=stage_||certificate_->slot_!=slot_||certificate_->artifact_!=report_.artifact_digest||certificate_->report_!=report_.report_digest){fail();return make_error(boolean_error_code::internal_invariant_error,stage_,"publish_unverified");}auto p=std::make_shared<published_artifact<Artifact>>(published_artifact<Artifact>{owner_,stage_,slot_,report_.artifact_digest,candidate_,report_,expected+1,std::move(prior)});auto generation=catalog.compare_and_publish(slot_,expected,owner_,p);if(!generation.has_value()){auto error=generation.error();fail();return error;}if(generation.value()!=p->generation){fail();return make_error(boolean_error_code::internal_invariant_error,stage_,"generation_mismatch");}certificate_->consumed_=true;for(auto&r:reservations_)r.commit();state_=transaction_state::published;candidate_.reset();return std::shared_ptr<const published_artifact<Artifact>>(std::move(p));}
};
} }
#endif
