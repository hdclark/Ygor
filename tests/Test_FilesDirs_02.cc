...
int main(int argc, char **argv){
    ...
    if(argc < 2) FUNCERR("This program requires a (test, dummy) file as input. It will output a copy to /tmp/filesdirstest2.out");
    ...
    FUNCINFO("The full path of the input filename is '" << fullpath << "'");
    return 0;

    //Test the Get_Piece_of_Binary_File() function by copying a file, chunk by chunk, to a duplicate in /tmp/
    if(!Does_File_Exist_And_Can_Be_Read(FilenameIn)) FUNCERR("Unable to open input file! Bailing");
