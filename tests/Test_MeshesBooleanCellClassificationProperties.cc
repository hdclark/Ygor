...
static std::array<std::uint64_t,3> classification_geometry_storage(const validated_operands<T,I>&v){
  ...
  std::function<std::uint64_t(std::uint64_t)>nodes=[&](std::uint64_t count){return count==0?std::uint64_t(0):count<=8?std::uint64_t(1):std::uint64_t(1)+nodes(count/2)+nodes(count-count/2);};const auto common=facets*sizeof(sourced_exact_facet3)+triangles*sizeof(sourced_exact_triangle3)+ring_entries*(sizeof(exact_point3)+sizeof(exact_point2)+sizeof(original_vertex_id)+sizeof(std::vector<facet_id>))+fan_members*sizeof(facet_id)+facets*(sizeof(exact_box3)+sizeof(std::size_t));const auto producer=common+triangles*(sizeof(exact_box3)+sizeof(std::size_t))+nodes(triangles)*sizeof(sourced_exact_ray_index_node3);return {producer,common,duplicated_payload};
}
static void facet_storage_accounting(){
  auto measure=[](std::size_t subdivisions){auto a=subdivided_square_prism<double,std::uint32_t>(subdivisions);fv_surface_mesh<double,std::uint32_t>b;boolean_options options;options.tracing.collect_noncanonical_timings=true;auto c=classification_test::context(a,b,classification_test::registry(),operation::regularized_union,options);auto result=classify_arrangement_cells(*c);require(result.has_value(),"highly triangulated classification");classification_oracle(*result.value()->payload);const auto expected=classification_geometry_storage(*result.value()->payload->validated->payload);const auto&stage=c->performance()->stage(boolean_stage::cell_classification);require(stage.producer.resource(resource_kind::stage_private_bytes)==expected[0],"producer facet classification storage accounting");require(stage.verifier.resource(resource_kind::verifier_scratch_bytes)==expected[1],"verifier facet classification storage accounting");require(stage.producer.value(performance_counter::exact_ray_facet_tests)<=stage.producer.value(performance_counter::ray_box_candidates),"producer exact tests limited to ray-bound candidates");require(stage.producer.value(performance_counter::exact_ray_facet_tests)<stage.producer.value(performance_counter::classification_source_triangles),"producer ray index reduces exhaustive triangle tests");require(stage.verifier.value(performance_counter::reconstructed_rays)>0&&stage.verifier.value(performance_counter::exact_ray_facet_tests)<=stage.verifier.value(performance_counter::ray_box_candidates),"verifier independently reconstructs only facet-bound candidates");return expected;};
