#pragma once

#include "CleanupFixtures.h"
#include "YgorMeshesBooleanBounded/OutputAssembly.h"

#include <cstdint>
#include <memory>
#include <stdexcept>
#include <string>

namespace assembly_tests {
namespace bounded = ygor::mesh_boolean::bounded;
using scalar = double;
using index_type = std::uint32_t;

using candidate_type = bounded::assembled_output_candidate<scalar, index_type>;

inline void require(bool value, const char *message) {
  if (!value)
    throw std::runtime_error(message);
}

struct assembly_fixture final {
  cleanup_tests::cleanup_fixture cleanup;
  std::shared_ptr<const candidate_type> output;
};

assembly_fixture build_assembly_fixture(
    scalar x0, scalar y0, scalar z0, scalar x1, scalar y1, scalar z1,
    scalar X0, scalar Y0, scalar Z0, scalar X1, scalar Y1, scalar Z1,
    boolean_operation operation);

std::string diagnostic(const bounded_boolean_error &error);

} // namespace assembly_tests
