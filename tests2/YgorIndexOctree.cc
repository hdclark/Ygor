...
TEST_CASE( "octree bbox operations" ){
    ...
    SUBCASE("bbox has_extent"){
        octree<double>::bbox point_box(vec3<double>(1.0, 2.0, 3.0), vec3<double>(1.0, 2.0, 3.0));
        octree<double>::bbox volume_box(vec3<double>(1.0, 2.0, 3.0), vec3<double>(2.0, 3.0, 4.0));
...
TEST_CASE( "octree insertion and search" ){
    ...
    SUBCASE("insert bbox and query mixed entries"){
        ...
        const octree<double>::bbox bb(vec3<double>(1.0, 1.0, 1.0), vec3<double>(2.0, 2.0, 2.0));
        ...
        const octree<double>::bbox query_box(vec3<double>(-0.5, -0.5, -0.5), vec3<double>(1.5, 1.5, 1.5));
