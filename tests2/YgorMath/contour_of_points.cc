...
TEST_CASE( "contour_of_points integration and utilities" ){
    ...
    SUBCASE("simple scalar contour line integral"){
        contour_of_points<double> contour({ vec3<double>(5.0, 10.0, 0.0),
                                            vec3<double>(0.0, 10.0, 0.0),
                                            vec3<double>(0.0,  0.0, 0.0),
                                            vec3<double>(5.0,  0.0, 0.0) });
        contour.closed = true;
        auto kernel = [](const vec3<double> &, const vec3<double> &, const vec3<double> &, const vec3<double> &) -> double {
            return 1.0;
        };
        const double result = contour.Integrate_Simple_Scalar_Kernel(kernel);
        const double expected = 30.0;
        double tolerance = 0.01;
        REQUIRE(std::abs(result - expected) < tolerance);
    }

    SUBCASE("textbook scalar contour line integral (Q11 p1107)"){
        contour_of_points<double> contour({ vec3<double>(0.0, 0.0, 0.0),
                                            vec3<double>(1.0, 2.0, 3.0) });
        contour.closed = false;
        auto kernel = [](const vec3<double> &r, const vec3<double> &, const vec3<double> &, const vec3<double> &) -> double {
    ...
    SUBCASE("contour equality"){
        ...
        REQUIRE(contour.eps_equal(contour_same, eps));
        REQUIRE(contour2.eps_equal(contour2_same, eps));
