...
TEST_CASE( "Plane_Orthogonal_Regression" ){
    ...
    SUBCASE("fit plane to some coplanar (z=5) points"){
    ...
    SUBCASE("fit plane to some coplanar (x=5) points"){
        std::vector< vec3<double> > thepoints{
    ...
    SUBCASE("fit plane to some coplanar (y=5) points"){
        std::vector< vec3<double> > thepoints{
    ...
    SUBCASE("cube data may throw or produce ambiguous result"){
        ...
        // depending on numerical factors. It is preferred that the implementation will
        // throw an exception, but this cannot be counted upon.
        bool acceptable = false;
        try{
            const auto theplane = Plane_Orthogonal_Regression(thepoints);

            // If it doesn't throw, just verify the normal is at least a unit vector.
            acceptable = (std::abs(theplane.N_0.length() - 1.0) < eps);
